...
export const Header: React.FC<HeaderProps> = ({ language, setLanguage }) => {
  return (
    <header className="w-full max-w-4xl mx-auto p-4 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
      ...
      <div className="flex flex-wrap items-center justify-center gap-1 bg-gray-800 rounded-lg p-1">
        <button
          onClick={() => setLanguage(Language.ENGLISH)}
          className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${language === Language.ENGLISH ? 'bg-indigo-600 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
        >
          English
        </button>
        <button
          onClick={() => setLanguage(Language.HEBREW)}
          className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${language === Language.HEBREW ? 'bg-indigo-600 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
        >
          עברית
        </button>
        <button
